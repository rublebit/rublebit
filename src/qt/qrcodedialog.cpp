...
{
    QString ret = QString("rublebit:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non RUBIT unit input in lnReqAmount, we generate the URI with RUBIT as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(RubleBitUnits::format(RubleBitUnits::RUBIT, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
