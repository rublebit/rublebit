...
const int RUBLEBIT_IPC_CONNECT_TIMEOUT = 1000; // milliseconds
const QString RUBLEBIT_IPC_PREFIX("rublebit:");
...
bool PaymentServer::ipcSendCommandLine()
{
    ...
    {
        if (!args[i].startsWith(RUBLEBIT_IPC_PREFIX, Qt::CaseInsensitive))
            continue;
    ...
    foreach (const QString& arg, savedPaymentRequests)
    {
        ...
        if (!socket->waitForConnected(RUBLEBIT_IPC_CONNECT_TIMEOUT))
            return false;
        ...
        socket->waitForBytesWritten(RUBLEBIT_IPC_CONNECT_TIMEOUT);
...
{
    // Install global event filter to catch QFileOpenEvents on the mac (sent when you click rublebit: links)
    ...
    if (!uriServer->listen(name))
        qDebug() << tr("Cannot start rublebit: click-to-pay handler");
    else
...
{
    // clicking on rublebit: URLs creates FileOpen events on the Mac:
