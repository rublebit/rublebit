...
#if defined(NDEBUG)
# error "RubleBit cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x9cacb93d3af9b62cd8836cd414046457d722056175f63c630ee8cbafb2637d67");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // RubleBit: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "RubleBit Signed Message:\n";
...
{
    // RubleBit: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 1000000); // RubleBit: 1000k blocks in ~2 year

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 10 * 30; // RubleBit: 5 minutes
static const int64 nTargetSpacing = 1 * 60; // RubleBit: 60 Seconds.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // RubleBit: This fixes an issue where a 51% attack can change difficulty at will.
...
void ThreadScriptCheck() {
    RenameThread("rublebit-scriptch");
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // RubleBit: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "07/Nov/2015 RUBLEBIT.info";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 1 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("6d21b1b63ae6266ce4716835f7da5dceff5287fdbba945ee28f89a026d28e49d56301581f9a8a265b89d35f5047fa6710c7b7b008856b190154591d72cdef70022") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1446900000;
        ...
        assert(block.hashMerkleRoot == uint256("0xf5ae73b692b4c99978033292f30e521239bbe9641585e09eb0b9ea3827021864"));
...
unsigned char pchMessageStart[4] = { 0xc3, 0xd2, 0xd1, 0xbd }; // RubleBit: increase each by adding 2 to rublebit's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("RubleBitMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("RubleBitMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static RubleBitMiner(CWallet *pwallet)
{
    printf("RubleBitMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("rublebit-miner");
    ...
    try { loop {
        ...
        printf("Running RubleBitMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("RubleBitMiner terminated\n");
        throw;
    }
}

void GenerateRubleBits(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&RubleBitMiner, pwallet));
}
